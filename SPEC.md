...
| ~~Amazon Julia (FR)~~ / ~~Amazon Julia (DE)~~ / ~~Amazon Markus (FR)~~ / ~~Amazon Markus (DE)~~ / ~~Geld verliehen/geliehen~~ | receivable | Außenstände | – | **Superseded by Außenstände above (Sept 2026) — not deleted from the schema, `tracked: false` instead.** Historical transactions still on these accounts need migrating onto Außenstände (with a proper claim tag) before these can be considered fully retired; not yet done as of this decision, tracked in DEVLOG. |
