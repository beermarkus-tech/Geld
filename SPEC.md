...
**Account info moved into the sidebar's bottom (Sept 2026, Markus)** — the signed-in user's profile picture (from Google sign-in) plus an "Abmelden" (sign-out) button now sit at the bottom of the tablet sidebar, below the nav items, rather than in the header. The header used to show the e-mail address as plain text; that's now only a tooltip on the Abmelden button, with the picture standing in for it visually. Phone has no sidebar to hold this, so the same picture + Abmelden pairing sits at the bottom of the phone's "More" sheet instead (§1b.2's "layout difference, not a capability difference" principle applied here too). If the account has no picture, a plain circle with the e-mail's first letter stands in.
...
- **Red** — alerts only, everywhere, always. **One explicit, deliberate exception (Sept 2026, Markus's own call, asked directly since it otherwise contradicts "everywhere, always"):** Verlauf's (§3b) Kategorie/Unterkategorie/row-total column background tint uses real red for its Ausgaben section specifically — not amber — while every other red-reserved-for-alerts rule elsewhere in the app is unchanged. A section *background tint* identifying which of Verlauf's three visual groups a row belongs to (Einnahmen/Ausgaben/Rücklagen) is read differently than a value's own color (the thing the "no red for category type" rule was actually protecting against), so this one spot is carved out by name rather than reopening the general rule.

### 1b.5 Light/dark mode
Both supported, following system preference by default with a manual override. **Corrected Sept 2026 (Markus):** the override lives as a sun/moon button in the header's top-right corner, not inside Settings — Markus wanted it available immediately, ahead of Settings existing at all, and it stayed there rather than moving once Settings does. Implemented via CSS custom properties for every semantic color (§1b.4) rather than hard-coded per-component colors, so a token's meaning (e.g. "alert red") maps to an appropriate concrete shade in each theme without every screen needing separate light/dark styling work. The override is remembered per device (localStorage), the same "not synced app data" treatment as the sidebar's collapsed state (§1b.2).
...
| Außenstände | receivable | Außenstände | – | **Resolved Sept 2026, replaces the old Amazon Julia (DE/FR), Amazon Markus (DE/FR), and Geld verliehen/geliehen accounts — one shared receivable account, not five; migration completed the same day.** Which specific claim or loan a transaction belongs to is carried by a `claim`-type grouping tag (§2.5), not by which account it sits on; direction (money owed growing vs. being repaid) follows the same sign-of-transfer convention as any other transfer (§2.6). The pinned panel (§3a) doesn't show this account's own lump balance directly — every currently-*open* tag touching it (its own total ≠ 0) gets its own dynamically-generated row instead, exactly the same "nets to zero, drops off the list" mechanism §3a already describes for loans, just now covering Amazon returns too. Markus's own examples: `Dirk` (an informal loan), `Amazon`/`Zalando` (a return in flight) — deliberately coarse-grained tags only (which retailer/person), with any further detail (which specific order) left to the `detail` free-text field, not a second tag. |
| CPAM | receivable | Außenstände | – | money to claim back; consists of individual bookings (see §2.4). Kept as its own account, not folded into Außenstände above — Markus: "will have a specific tracker later on in the app." |
| Reisekosten Airbus | receivable | Außenstände | – | travel expenses to be reimbursed; individual bookings roll up to a claim. Kept as its own account for the same reason as CPAM above. |
...
- **Einnahmen** — Gehalt Markus, Gehalt Julia, Sonderzahlungen, Kindergeld, Sonstige Einnahmen. **"Erstattungen" removed entirely (Sept 2026, Markus: "I do not recognize Erstattungen, please remove it from the app")** — was in the original Gsheet transcription above but not a category Markus actually uses; filtered out of every category picker and Verlauf's own display. The underlying Firestore document (and any old data that happens to reference it) isn't something this session can reach directly (no live Firestore access, per this project's standing rule) — harmless left alone as long as nothing in the app surfaces it, but a real delete/cleanup pass is a Settings-area (§3i) job once that screen exists, if anything still points to it.
...
**Confirmed column order (grid and phone-card layout alike):** Datum → Konto → Empfänger → Betrag → Kategorie → Unterkategorie → Details → Tags.
- **Konto — one merged column, not two (revised Sept 2026, superseding this section's original Konto 1/Konto 2 split and the mockup's split-column layout below):** shows the transaction's account(s) as a single label with a directional arrow between them for a transfer (e.g. "BNP-Konto → Livret A Sparen"), or just the one account for a plain income/expense row. The original two-column design existed mainly so each account could be its own filterable grid column; once the real filtering need got worked through together (see the filtering section below), that turned out not to be what "filter by account" actually has to mean, so the extra column split wasn't earning its keep — one column reads closer to the Gsheet ledger Markus already knows, which is what he asked for once that was clear. **Still two separate account fields underneath** (`fromAccountId`/`toAccountId`, unchanged, §2.6) — merging the display doesn't merge the data; exactly how editing a merged cell works (e.g. tapping it to reveal both account pickers) is an implementation detail for whenever manual entry is built, not resolved here.
...
**Konto column derivation (from `fromAccountId`/`toAccountId`, nothing new stored):** the primary account shown is `fromAccountId` when present (an outflow: expense or transfer), otherwise `toAccountId` (pure income). For an actual transfer, the second account follows after a bold, tinted arrow that always points from the outflow side to the inflow side — so it reads in the direction the money actually moved, never a fixed left-to-right convention. No arrow, just the one account, for a plain income/expense row.

**Filtering by an allocation tag reorders the Konto column (Sept 2026, Markus)** — when the active tag filter is an `allocation`-class tag (e.g. Sparen Sophia), every row shown always lists that tag's own `reconciliationTargetAccountIds` account first/left in the Konto column, with the arrow mirrored (pointing left) when money actually flowed into it from the right-hand side rather than out of it. This is display ordering only, not a change to which account is `fromAccountId`/`toAccountId` — the point is that with the savings/investment side pinned to one edge, the arrow direction alone tells Markus at a glance whether a given row was a contribution into that tag's account or a withdrawal out of it, instead of having to read which side each row happens to put it on.
...
**Reference mockup:** `mockup-konten.html` includes a live, working demo of the cascading Kategorie→Unterkategorie dropdown behavior (change Kategorie on the "Market Magasin" row to see Unterkategorie clear and flag itself), plus the expand-affordance and corrected status-dot placement described above. **Its Konto 1/Konto 2 split-column layout is superseded** by the merged Konto column above — everything else it demonstrates still applies.
...
**Loans / receivables — replaces the old "Verliehen" column entirely, and now closed the same way as any other Außenstände (§3g):** tracked structurally via the `receivable` account group + a `claim`-type grouping tag on both the lending line and the repayment line. Example: lending a friend money — tag the outflow with e.g. `Dirk`; when repaid, tag that inflow with the same tag; the moment the two net to zero, the open-status dot clears itself. **No manual linking step exists** — the link-picker mechanism from an earlier pass of this spec is retired entirely, replaced by the same tag-based zero-sum closure used for CPAM, Reisekosten, and Amazon returns (§3g), rather than being its own separate mechanism. **Loans and Amazon returns now share the one `Außenstände` account (§2.2, Sept 2026)**, not one account each — a claim tag alone (`Dirk`, `Amazon`, `Zalando`, deliberately coarse-grained; finer detail belongs in that line's `detail` free text, not a second tag) is what distinguishes one open claim from another. CPAM and Reisekosten Airbus stay their own accounts, not folded in, since each is planned to get its own dedicated tracker screen later.
...
- **Per-field filters:** every column independently filterable (date, category, subcategory, tag "has this tag," status, etc.) via AG Grid's native column filters, restyled to match the app's visual language (§1b.4) rather than left in AG Grid's generic default appearance.
- **Filtering by account is its own filter, not a plain column filter on Konto (resolved Sept 2026):** Konto's content is *derived* per row (above) — the same real account can be the primary account on one row and the arrow's far side on another, depending only on which way the money moved that day, so a literal "Konto column == DKB-Konto" filter would silently miss half of DKB's own history. Instead, picking an account from a dedicated account filter matches every transaction where that account is either `fromAccountId` or `toAccountId`, and — the actual point of it — re-displays each matching row **from that account's own perspective**: the *other* account shown as the counterpart, and Betrag re-signed relative to the chosen account (negative when money left it, positive when it arrived), using the same magnitude-by-position rule as `balance()` (§2.6/§2.8), never the raw stored `amountCents`. In effect a single-account register view — closer to what Markus actually reaches for than a literal spreadsheet column filter, and confirmed as more elegant than the old Gsheet's own two-rows-per-transfer approach.
- **Filtering by tag (any tag, added Sept 2026) is the same dedicated-filter idea, not a plain column filter either — but reachable two ways, deliberately, not one:** click any tag chip anywhere in the grid (or a tag row in the pinned panel, §3a's panel section), or use the Tags column's own header filter (a plain "contains" text search — the one column, so far, that has one; every column was found to be missing this entirely, a real spec/build gap, not by design). Both set the same underlying filter, kept in sync in both directions.
  - **The filtered total shown alongside it has to handle direction differently depending on what the tag actually is — resolved directly with Markus, Sept 2026:** a tag genuinely tied to the shared `Außenstände` account (§2.2 — a claim or loan) is summed the *position*-based way (§2.8's own account-position rule, generalized): + when money arrived in Außenstände, − when it left, so a fully-settled claim correctly totals to zero rather than always reading zero-by-construction (the bug a plain signed sum would have — every line touching a claim tag is, definitionally, one leg of a transfer). Any other tag (a trip, an expense breakdown, an allocation tag) sums each tagged line's own natural sign directly, and a transfer between two accounts *neither* of which is Außenstände is excluded from the total entirely — an internal reallocation between real accounts isn't spending and isn't a claim movement either.
...
**Display resolutions from real first-build usage (Sept 2026):**
- **Fixed subcategory order within each group, and a fixed order for the Rücklagen section** — not alphabetical (an earlier, unconfirmed assumption). Groups themselves stay in §2.4's own order (Einnahmen, Wohnen, Kommunikation, Mobilität, Lebenshaltung, Gesundheit, Hobbys, Sonstiges). Within Wohnen: Hauskredit, Nebenkosten, Instandhaltung, Einrichtung, Garten. Kommunikation: Internet, Fernsehen, Telefon. Mobilität: Firmenwagen, Autoversicherung, Wartung, Tanken, Gebühren. Lebenshaltung: Lebensmittel & Haushalt, Kantine, Ausgehen, Klamotten Markus, Klamotten Julia, Klamotten Sophia, Ausstattung Sophia, Allgemein, Haustiere, Versicherungen. Gesundheit: Medizin, Arztkosten, Krankenkasse. Hobbys: Hobbys Julia, Hobbys Markus, Hobbys Sophia. Sonstiges: Urlaube, Geschenke, Sonderausgaben, Steuerausgaben, Rente, Sonstige Ausgaben. Rücklagen (the allocation-tag section): Sparen Familie, Sparen Sophia, Sparen Julia, Anlage Familie, Anlage Sophia, Rücklagen Steuern, Tagesgeld.
- **Kategorie column: rotated text, 90° counterclockwise, vertically centered and bold** within its own merged (spanned) cell — narrower than Unterkategorie now that its text runs vertically, with the freed width going to Unterkategorie and the row-total column instead (both were clipping real figures at the old widths).
- **The row-total column drops its Prog/Plan1/Plan0 text label** — once font color (below) distinguishes the three rows, a repeated text label is redundant; the column shows just the yearly € figure.
- **Font color by row and by that month's open/closed state:** Plan0 is always grey (it's the fixed reference, never the "current" number). Plan1 is black in an open month (still the actively relevant forecast) and grey once that month closes (superseded by the real actual). Prog is the mirror image — grey while open (a placeholder echo of Plan1), black once closed (now the real number).
- **Prog's own row gets a light grey background tint on any closed month's cells specifically** (not the whole row, and not Plan1/Plan0's cells — corrected Sept 2026, was initially built on Plan0 by mistake) — a second, independent visual cue (alongside the grey text) that a month is done and its numbers are final. Makes sense specifically on Prog: that's the row that just became the definitive, no-longer-a-mirror number for that month.
- **No gridline between the three sibling rows (Prog/Plan1/Plan0) of one category/allocation-tag block** — they read as one visual unit; the line between one block and the next stays.
...
**Reference mockup:** `mockup-verlauf.html` — tablet grid (three sticky columns with live, dynamically-recomputed rowspan; independent Plan1/Plan0 breakdown toggles; global show/hide controls) and phone's one-month card view, side by side, light/dark toggle included. Predates the display resolutions immediately above — those aren't reflected in the mockup file itself, only in this text.
