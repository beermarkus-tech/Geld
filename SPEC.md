...
- **Backend: Firebase Auth + Firestore + Firebase Hosting**, as already assumed throughout this spec — single-admin auth simplifies the security rules considerably relative to the source lessons-learned material (§1a).
...
- `balance(allocationTagId, asOfDate)` = Σ amountCents of transactions tagged with it, up to that date
