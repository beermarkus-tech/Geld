...
- **2026 so far migrated too** (added Sept 2026, Markus's decision — the "Geld 2026" sheet is in active use): its transactions to date and its Plan0/Plan1 budgets, without the 2026 sheet's own opening rows. Checked so that the 2026 sheet's header totals come out of 2025 + 2026 combined. After the import, the Gsheet stops being the place new transactions are entered only once Phase 4's parallel run says so.
...
- Split-transaction UI and the auto-remainder mechanism (§3a) — separated from 1a deliberately (external review, Sept 2026): the most intricate interaction in the app, worth its own checkpoint rather than being bundled with "does the grid work."
  - **Open design question, flagged by Markus Sept 2026, not yet resolved:** a split line is an individual booking in its own right, and must be able to route to a "virtual" account (e.g. ESOP, Aktien — the `investment-tracking` group, §2.2) directly, not just carry a category. Today `lines[]` (§2.6) has no account fields at all — Konto (`fromAccountId`/`toAccountId`) is fixed once, at the parent/transaction level, and only the economic categorization (`categoryId`) splits per line. A paycheck deposit where part of the gross salary routes straight into ESOP (never actually touching the bank account) can't be represented correctly under the current model — tagging that portion's category as "ESOP" is semantically wrong and wouldn't move ESOP's own computed balance at all. Needs deliberate design work, not a quick patch — see DEVLOG.md's open-items entry for the downstream impacts already spotted (`balance()`, the split invariant's meaning, Konto's display, transfer-leg matching, the migration scripts) before deciding how to extend the schema.
...
- **Real navigation shell (§1b.2)** — added here (gap found Sept 2026: spec.md fully designs this but no phase had ever actually scheduled building it), since this is the first phase where a third real destination screen exists. Before now, `App.jsx`'s plain Konten/Datenimport toggle was sufficient; it stops being sufficient once Verlauf and Planung both need to be reachable too. Bottom nav bar (phone, 5 items) / left sidebar (tablet, all items, collapsible via a burger button next to "Geld" — Markus's own addition, §1b.2) per §1b.2. **Resolved (Sept 2026, Markus):** every item gets a real nav entry from the start, including screens not built until later phases (Dashboard, Quickview, Fortschritt, etc.) — each shows a plain "kommt noch" placeholder rather than being left out or added incrementally. **Also resolved:** the global year selector (§1b.2a) moves into this shared header now, in this phase, rather than waiting for Verlauf to need it — `Konten.jsx`'s own year buttons come out of its toolbar at the same time.
